...
export const SelectPlanScreen: React.FC<{ route: any; navigation: any }> = ({ route, navigation }) => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const orderRes = await fetch(`${API_BASE}/api/parent/payment/order`, {
        method: 'POST',
        headers: {
  ...
  const verifyPayment = async (orderId: string, paymentId: string, signature: string, amount: number) => {
    try {
      ...
      const verifyRes = await fetch(`${API_BASE}/api/parent/payment/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleWalletPayment = async (amount: number) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/parent/wallet/pay`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    <View style={styles.container}>
      ...
      <View style={styles.plansContainer}>
        ...
        <TouchableOpacityPlan
          ...
          price="₹7,000 / qtr"
          desc="15% discount. Sibling priorities and priority matching enabled."
