...
import { API_BASE, adminApi } from '../services/api';

const BACKEND_URL = API_BASE.SUPER_ADMIN;

export const AdminHomeScreen: React.FC = () => {
  const [token, setToken] = useState<string | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [activeTab, setActiveTab] = useState<'stats' | 'users' | 'vehicles' | 'complaints' | 'ai' | 'workflow'>('workflow');
  ...
  const [workflowLogs, setWorkflowLogs] = useState<string[]>([
    '[INIT] 5 microservice nodes connected & verified healthy.',
    '[PORTAL] Parent Portal (Port 8085) - Live Ride Pass active.',
    '[CORE] Super Admin Orchestrator (Port 8082) - KYC & Route matched.',
    '[FLEET] Cab Owner Portal (Port 8083) - Force Traveler assigned.',
    '[BEACON] Conductor App (Port 8084) - GPS streaming (24ms).',
    '[TRACK] Real-time Geofence 0.5-mile parent radar active.'
  ]);

  const handleSimulateError = (service: string | null) => {
    setWorkflowErrorService(service);
    const timeStr = new Date().toLocaleTimeString();
    if (service) {
      setWorkflowLogs(prev => [
        `[${timeStr}] 🚨 ALERT: Node [${service.toUpperCase()}] degradation simulated.`,
        ...prev.slice(0, 5)
      ]);
    } else {
      setWorkflowLogs(prev => [
        `[${timeStr}] ✅ RECOVERY: All nodes restored to 200 OK healthy.`,
        ...prev.slice(0, 5)
      ]);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <TouchableOpacity
            style={[styles.tabButton, activeTab === 'workflow' && styles.activeTabButton]}
            onPress={() => setActiveTab('workflow')}
          >
            <Text style={[styles.tabLabel, activeTab === 'workflow' && styles.activeTabLabel]}>🔀 Workflow (Live)</Text>
          </TouchableOpacity>
          <TouchableOpacity
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent}>
        {/* TAB 0: WORKFLOW & ROUTE FLOW MONITOR */}
        {activeTab === 'workflow' && (
          <View style={{ marginBottom: 20 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <View>
                <Text style={styles.sectionTitle}>Multi-Service Pipeline Monitor</Text>
                <Text style={{ color: '#94a3b8', fontSize: 10, marginTop: -10, marginBottom: 6 }}>
                  Directional microservice handshakes & health telemetry
                </Text>
              </View>
              {workflowErrorService ? (
                <Button 
                  ...
                  buttonColor="#10b981" 
                  compact 
                  onPress={() => handleSimulateError(null)}
                ...
                <Chip style={{ backgroundColor: 'rgba(16,185,129,0.15)' }} textStyle={{ color: '#10b981', fontSize: 9, fontWeight: 'bold' }}>
                  ● 200 OK
                </Chip>
              )}
            </View>

            {/* Microservice Flow Cards */}
            <View style={{ marginBottom: 16 }}>
              
              {/* Node 1: Parent Portal */}
              <Card style={[styles.listItemCard, workflowErrorService === 'parent' && { borderColor: '#ef4444', borderWidth: 1.5 }]}>
                <Card.Content style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Avatar.Icon size={34} icon="cellphone" style={{ backgroundColor: workflowErrorService === 'parent' ? '#ef4444' : '#6366f1', marginRight: 10 }} />
                    <View>
                      <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>1. Parent Portal</Text>
                      <Text style={{ color: '#94a3b8', fontSize: 10 }}>Port 8085 (Spring Boot) • Pass #SP-9921</Text>
                    </View>
                  </View>
                  <Chip 
                    style={{ backgroundColor: workflowErrorService === 'parent' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.15)' }}
                    textStyle={{ color: workflowErrorService === 'parent' ? '#ef4444' : '#10b981', fontSize: 9, fontWeight: 'bold' }}
                  >
                    {workflowErrorService === 'parent' ? 'DEGRADED' : 'HEALTHY'}
                  </Chip>
                </Card.Content>
              </Card>
              ...
              <View style={{ alignItems: 'center', marginVertical: 4 }}>
                <Text style={{ color: '#6366f1', fontSize: 12, fontWeight: 'bold' }}>↓ Handshake: Ride Reservation (JSON)</Text>
              </View>

              {/* Node 2: Super Admin Core */}
              <Card style={[styles.listItemCard, workflowErrorService === 'admin' && { borderColor: '#ef4444', borderWidth: 1.5 }]}>
                <Card.Content style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Avatar.Icon size={34} icon="shield-check" style={{ backgroundColor: workflowErrorService === 'admin' ? '#ef4444' : '#0284c7', marginRight: 10 }} />
                    <View>
                      <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>2. Super Admin Core</Text>
                      <Text style={{ color: '#94a3b8', fontSize: 10 }}>Port 8082 • KYC & AI Route Matcher</Text>
                    </View>
                  </View>
                  <Chip 
                    style={{ backgroundColor: workflowErrorService === 'admin' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.15)' }}
                    textStyle={{ color: workflowErrorService === 'admin' ? '#ef4444' : '#10b981', fontSize: 9, fontWeight: 'bold' }}
                  >
                    {workflowErrorService === 'admin' ? 'DEGRADED' : 'HEALTHY'}
                  </Chip>
                </Card.Content>
              </Card>
              ...
              {/* Down Arrow */}
              <View style={{ alignItems: 'center', marginVertical: 4 }}>
                <Text style={{ color: '#6366f1', fontSize: 12, fontWeight: 'bold' }}>↓ Dispatch: Fleet Assignment</Text>
              </View>

              {/* Node 3: Cab Owner Portal */}
              <Card style={[styles.listItemCard, workflowErrorService === 'owner' && { borderColor: '#ef4444', borderWidth: 1.5 }]}>
                <Card.Content style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Avatar.Icon size={34} icon="account-group" style={{ backgroundColor: workflowErrorService === 'owner' ? '#ef4444' : '#0d9488', marginRight: 10 }} />
                    <View>
                      <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>3. Cab Owner Fleet</Text>
                      <Text style={{ color: '#94a3b8', fontSize: 10 }}>Port 8083 • Van TN-12-AE-3940</Text>
                    </View>
                  </View>
                  <Chip 
                    style={{ backgroundColor: workflowErrorService === 'owner' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.15)' }}
                    textStyle={{ color: workflowErrorService === 'owner' ? '#ef4444' : '#10b981', fontSize: 9, fontWeight: 'bold' }}
                  >
                    {workflowErrorService === 'owner' ? 'DEGRADED' : 'HEALTHY'}
                  </Chip>
                </Card.Content>
              </Card>
              ...
              {/* Down Arrow */}
              <View style={{ alignItems: 'center', marginVertical: 4 }}>
                <Text style={{ color: '#6366f1', fontSize: 12, fontWeight: 'bold' }}>↓ Checklist: Student Attendance</Text>
              </View>

              {/* Node 4: Conductor & Driver Mobile */}
              <Card style={[styles.listItemCard, workflowErrorService === 'driver' && { borderColor: '#ef4444', borderWidth: 1.5 }]}>
                <Card.Content style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Avatar.Icon size={34} icon="steering" style={{ backgroundColor: workflowErrorService === 'driver' ? '#ef4444' : '#10b981', marginRight: 10 }} />
                    <View>
                      <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>4. Conductor Mobile</Text>
                      <Text style={{ color: '#94a3b8', fontSize: 10 }}>Port 8084 • GPS Beacon Active</Text>
                    </View>
                  </View>
                  <Chip 
                    style={{ backgroundColor: workflowErrorService === 'driver' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.15)' }}
                    textStyle={{ color: workflowErrorService === 'driver' ? '#ef4444' : '#10b981', fontSize: 9, fontWeight: 'bold' }}
                  >
                    {workflowErrorService === 'driver' ? 'DEGRADED' : 'HEALTHY'}
                  </Chip>
                </Card.Content>
              </Card>
              ...
              {/* Down Arrow */}
              <View style={{ alignItems: 'center', marginVertical: 4 }}>
                <Text style={{ color: '#6366f1', fontSize: 12, fontWeight: 'bold' }}>↓ Live Feed: Coordinates Broadcast</Text>
              </View>

              {/* Node 5: Real-Time Tracking */}
              <Card style={[styles.listItemCard, workflowErrorService === 'tracking' && { borderColor: '#ef4444', borderWidth: 1.5 }]}>
                <Card.Content style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Avatar.Icon size={34} icon="map-marker-radius" style={{ backgroundColor: workflowErrorService === 'tracking' ? '#ef4444' : '#9333ea', marginRight: 10 }} />
                    <View>
                      <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>5. Live Tracking Map</Text>
                      <Text style={{ color: '#94a3b8', fontSize: 10 }}>WebSocket Feed • 0.5-Mile Geofence</Text>
                    </View>
                  </View>
                  <Chip 
                    style={{ backgroundColor: workflowErrorService === 'tracking' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.15)' }}
                    textStyle={{ color: workflowErrorService === 'tracking' ? '#ef4444' : '#10b981', fontSize: 9, fontWeight: 'bold' }}
                  >
                    {workflowErrorService === 'tracking' ? 'DEGRADED' : 'HEALTHY'}
                  </Chip>
                </Card.Content>
              </Card>
            ...
            {/* Simulation Controls */}
            <Card style={[styles.listItemCard, { marginBottom: 16 }]}>
              <Card.Content>
                <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12, marginBottom: 8 }}>
                  ⚡ Simulate Microservice Fault
                </Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
                  <Button 
                    mode={workflowErrorService === 'parent' ? 'contained' : 'outlined'} 
                    compact 
                    buttonColor={workflowErrorService === 'parent' ? '#ef4444' : undefined}
                    textColor={workflowErrorService === 'parent' ? '#fff' : '#ef4444'}
                    onPress={() => handleSimulateError(workflowErrorService === 'parent' ? null : 'parent')}
                    labelStyle={{ fontSize: 9 }}
                  >
                    Parent SOS
                  </Button>
                  <Button 
                    mode={workflowErrorService === 'admin' ? 'contained' : 'outlined'} 
                    compact 
                    buttonColor={workflowErrorService === 'admin' ? '#ef4444' : undefined}
                    textColor={workflowErrorService === 'admin' ? '#fff' : '#ef4444'}
                    onPress={() => handleSimulateError(workflowErrorService === 'admin' ? null : 'admin')}
                    labelStyle={{ fontSize: 9 }}
                  >
                    Admin Lag
                  </Button>
                  <Button 
                    mode={workflowErrorService === 'owner' ? 'contained' : 'outlined'} 
                    compact 
                    buttonColor={workflowErrorService === 'owner' ? '#ef4444' : undefined}
                    textColor={workflowErrorService === 'owner' ? '#fff' : '#ef4444'}
                    onPress={() => handleSimulateError(workflowErrorService === 'owner' ? null : 'owner')}
                    labelStyle={{ fontSize: 9 }}
                  >
                    Fleet Drop
                  </Button>
                  <Button 
                    mode={workflowErrorService === 'driver' ? 'contained' : 'outlined'} 
                    compact 
                    buttonColor={workflowErrorService === 'driver' ? '#ef4444' : undefined}
                    textColor={workflowErrorService === 'driver' ? '#fff' : '#ef4444'}
                    onPress={() => handleSimulateError(workflowErrorService === 'driver' ? null : 'driver')}
                    labelStyle={{ fontSize: 9 }}
                  >
                    Driver GPS
                  </Button>
                  <Button 
                    mode={workflowErrorService === 'tracking' ? 'contained' : 'outlined'} 
                    compact 
                    buttonColor={workflowErrorService === 'tracking' ? '#ef4444' : undefined}
                    textColor={workflowErrorService === 'tracking' ? '#fff' : '#ef4444'}
                    onPress={() => handleSimulateError(workflowErrorService === 'tracking' ? null : 'tracking')}
                    labelStyle={{ fontSize: 9 }}
                  >
                    Socket Drop
                  </Button>
            ...
            <Card style={styles.listItemCard}>
              <Card.Content>
                <Text style={{ color: '#38bdf8', fontWeight: 'bold', fontSize: 11, marginBottom: 6 }}>
                  📟 Diagnostics Stream Console
                </Text>
                {workflowLogs.map((log, i) => (
                  <Text key={i} style={{ color: log.includes('ALERT') ? '#ef4444' : log.includes('RECOVERY') ? '#10b981' : '#94a3b8', fontSize: 9, fontFamily: 'monospace', marginVertical: 1 }}>
                    {log}
                  </Text>
                ))}
              </Card.Content>
