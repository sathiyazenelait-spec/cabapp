...
import { StyleSheet, View, TouchableOpacity, Alert, Linking, ScrollView } from 'react-native';
...
export const LiveTrackScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      fetch(`${API_BASE.PARENT}/api/parent/trip/location?driverId=${selectedDriver.id}`)
        .then(res => res.json())
        .then(data => {
          if (data && data.latitude && data.longitude) {
            setCoords({ x: data.latitude, y: data.longitude });
          }
        })
        .catch(() => {
          setCoords(prev => {
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            dispatch(toggleSOS());
            try {
              await parentApi.triggerSos({ driverId: selectedDriver.id, coords });
              Alert.alert('SOS Active', 'Emergency command center notified.');
            } catch {
              Alert.alert('SOS Active', 'Emergency signal transmitted.');
            }
          } 
        }
      ]
      ...
      <View style={styles.drawer}>
        ...
        <View style={styles.radarControlRow}>
          <View>
            ...
            <Text style={styles.radarSub}>Triggers push sound when cab is within distance</Text>
          </View>
